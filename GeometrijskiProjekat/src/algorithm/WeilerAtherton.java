...
public class WeilerAtherton {
	...
	public AlgorithmOutput doAlgorithms(ArrayList<Point2D> s,
			ArrayList<Point2D> c, boolean sInC, boolean cInS) {
		...
		IntersectionFinder.getInstance().findIntersections(subject, clip, sInC);
		...
		toReturn.setIntersectionsEntering(new ArrayList<Point2D>(
				intersectionsEntering));
		toReturn.setIntersectionsExiting(new ArrayList<Point2D>(
				intersectionsExiting));
		toReturn.setFinalIntersections(PolygonIntersectionsFinder.getInstance()
				.getPolygonIntersections(subject, clip, sInC, cInS));
		return toReturn;
	}
	...
	private void checkAndCorrectPoints(ArrayList<Point2D> points) {
		for (Point2D point : points) {
			while (pointsCheck.contains(point)) {
				point.setLocation(point.getX() + (Math.random() - 0.5) * 2e-3,
						point.getY() + (Math.random() - 0.5) * 2e-3);
			}
	...
	private class PointCheckComparator implements Comparator<Point2D> {
		...
		public int compare(Point2D o1, Point2D o2) {
			if (Math.abs(o1.getX() - o2.getX()) > 1e-5)
